...
const highlightColor = colorPicker(data?.data?.Page?.mediaList[0]?.user?.options?.profileColor ?? 'pink');
const LeftAlign = ({ frame, children }) => {
    return (<hstack spacing="0" padding="0" frame={frame}>
      {children}
      <spacer />
    </hstack>);
};
const Media = ({ mediaItem }) => {
    ...
    const { title, coverImage, status, id, chapters, episodes, nextAiringEpisode, } = media;
    const mediaType = MEDIA;
    const fullProgress = status === 'RELEASING' ? nextAiringEpisode?.episode : null;
    return (<MediaLink mediaId={id} mediaType={mediaType}>
      <hstack>
        <Poster img={media.coverImage.medium}/>
        <vstack spacing="0" frame="max">
          <Title title={title}/>
          <ProgressText progress={progress} total={episodes ?? chapters} nextEpisodeDate={nextAiringEpisode?.timeUntilAiring} nextEpisode={nextAiringEpisode?.episode} type={MEDIA}/>
          <Progress current={progress} max={episodes ?? chapters ?? (progress || 1) * 5} secondary={fullProgress}/>
        </vstack>
        <spacer />
      </hstack>
    </MediaLink>);
};
const MediaLink = ({ mediaId, mediaType, children, }) => {
...
const ProgressText = ({ progress, total, nextEpisode, nextEpisodeDate, type, }) => {
    ...
    return (<hstack padding="3,0,3,0">
      <text font={font} color={highlightColor}>
        {progress} / {total ?? '??'} {typeText}s
      </text>
      <spacer />
      {nextEpisode && (<text font={font} color={highlightColor}>
...
const Progress = ({ current, max, secondary, w, h, }) => {
    ...
    return (<zstack frame="4">
      <hstack>
        <rect frame={`${width},${height}`} color="white" corner="3"></rect>
        <spacer />
      </hstack>
      <vstack spacing="0">
        <ProgressBar current={current} max={max} width={width} height={firstBarHeight}/>
        {secondary && (<ProgressBar current={secondary} max={max} width={width} height={2}/>)}
      </vstack>
    </zstack>);
};
const ProgressBar = ({ current, max, width, height, color = highlightColor, }) => {
    return (<hstack>
      <rect frame={`${progress(width, current, max)},${height}`} color={color} corner="3"></rect>
      <spacer />
    </hstack>);
};
$render(<vstack padding="5" spacing="3">
    {mediaMap().map((i) => {
